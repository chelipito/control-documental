/* =========================================================
   ...
   - Lee la configuración (configuracion.json)
...
/* =========================================================
   Configuración editable (configuracion.json)
   ========================================================= */
const CFG_DEFAULT = {
  ...
  diasAvisoPlazo: 7,
  estadoAlEncontrar: 'aprobado'
};
let CFG = CFG_DEFAULT;
try{
  const r = await fetch('configuracion.json', {cache:'no-store'});
...
/* Estado que se asigna cuando se encuentra el archivo (configuracion.json -> estadoAlEncontrar).
   Puede ser "aprobado" (automático) o "recibido" (queda para revisión manual). */
const ESTADO_AUTO = ['aprobado','recibido'].includes(CFG.estadoAlEncontrar) ? CFG.estadoAlEncontrar : 'aprobado';
const ORDEN_ESTADO = {pendiente:0, recibido:1, aprobado:2};
const textoEstado = k => (ESTADOS.find(e=>e.k===k)||{t:k}).t;
...
function calcularCambios(p, ws, grupos, mapa){
  ...
  for(const w of ws){
    ...
    for(const d of p.docs){
      ...
      const actual = ORDEN_ESTADO[c.estado];
      if(c.estado !== 'observado' && actual !== undefined && actual < ORDEN_ESTADO[ESTADO_AUTO]) cambios.push({w, d, c, ruta, marcar:true});
...
function openSync(){
  ...
  const dibujar = () => {
    ...
    $('#syncBody').innerHTML = `
      ...
      <p class="sum">${nuevos ? `Se marcarán <b>${nuevos}</b> documentos como <b>${esc(textoEstado(ESTADO_AUTO))}</b>.` : 'No hay documentos nuevos para marcar.'}
      ...
      <p class="hint">${ESTADO_AUTO==='aprobado' ? 'Cada archivo encontrado deja su documento como aprobado. Los documentos que marcaste como observados no se cambian.' : 'Los documentos aprobados u observados no cambian de estado. La revisión sigue siendo tuya.'}</p>`;
    ...
    $('#sGo').textContent = nuevos ? `Marcar ${nuevos} como ${textoEstado(ESTADO_AUTO).toLowerCase()}s` : 'Guardar';
    lectura.resultado = r;
  };
    ...
    try{
      ...
      for(const x of lectura.resultado.cambios){
        ...
        campos[`docs.${x.d.key}`] = x.marcar
          ? {estado:ESTADO_AUTO, nota:x.c.nota||'', link:x.c.link||'', archivo:x.ruta, fecha:today(), por:(S.user?.email||'')+' (lectura de carpeta)'}
          : {...x.c, archivo:x.ruta};
      ...
      $('#dlgSync').close(); toast(n ? `${n} documentos marcados como ${textoEstado(ESTADO_AUTO).toLowerCase()}s` : 'Lectura guardada');
